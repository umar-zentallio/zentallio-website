...
async function postNote(refName, lead) {
  try {
    await fetch(`${URL}/api/resource/Comment`, {
      ...
      body: JSON.stringify({
        ...
        content: qualificationNote(lead),
      }),
    });
  } catch (_) {
    /* best-effort timeline note */
  }
}
...
async function upsertLead(lead) {
  ...
  try {
    ...
    if (existing) {
      ...
      return { ok: true, name: existing, updated: true };
    }
    ...
    const name = j.data && j.data.name;
    if (name) await postNote(name, lead); // context brief on first create only
...
module.exports = { enabled, upsertLead, mapToRadar, statusFor, industryFor };
